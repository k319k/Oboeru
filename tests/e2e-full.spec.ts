...
test('full flow: seed → top → practice complete → manage round-trip → top', async ({
	...
	// --- TOP: seed data listed, select chapter, start ---
	await page.goto('/');
	...
	// --- PRACTICE: sentence 1 passes (recording → manual stop → transcribe) ---
	...
	// Retry: recording restarts → manual stop → pass
	await expect(page.getByTestId('sentence-recording')).toBeVisible({ timeout: 5000 });
	await page.getByTestId('stop-recording-btn').click();
	...
	const e2eChapter = page.locator('button.chapter-item', { hasText: 'E2Eチャプター' });
...
test('TTS end event fires: show → tts → hidden → transcribing', async ({ page }) => {
	// fake-media はトーン音声のため無音自動停止は発火しない。手動停止ボタンを基本とする(plan T5 手順4)
	await seedFull(page);
	await mockTts(page);
	...
	// recording starts. This verifies the end-event transition without
	// asserting on any audio content.
